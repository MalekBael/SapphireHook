...
namespace {
    ...
    // Side-effect analyzer: pulls minimal fields needed for correlation without UI duplication.
        ...
        const uint8_t* payload, size_t payloadLen)
    {
        ...
        // Client ActionRequest (opcode 0x0196) structure: see ClientZoneDef.h
        ...
        // Server ActionResult / ActionResult1 (0x0146 / 0x0147) – keep request id for potential UI augmentation
        else if (!outgoing && (opcode == 0x0146 || opcode == 0x0147) && payloadLen >= 0x18) {
            // RequestId at +0x10, not displayed here (registry handles display)
        ...
        size_t payloadLen)
    {
        ...
        // Try given direction, then inverted (captures possible mis-labeled direction)
...
namespace {
    ...
    static bool ExportToJsonAs(const HookPacket& hp, const std::string& filepath) {
        try {
            ...
            f << "  \"header\": { \"size\": " << P.size << ", \"connType\": " << P.connType
                << ", \"segCount\": " << P.segCount << ", \"isCompressed\": " << (unsigned)P.isCompressed << " },\n";
            f << "  \"segments\": [\n";
            for (size_t i = 0; i < segs.size(); ++i) {
                const auto& s = segs[i];
    ...
    static bool ExportToPcapAs(const HookPacket& hp, const std::string& filepath) {
        try {
            ...
            std::ofstream f(p, std::ios::binary);
            if (!f) return false;
            struct GH { uint32_t magic; uint16_t vmaj, vmin; int32_t thiszone; uint32_t sigfigs; uint32_t snaplen; uint32_t network; }
            gh{ 0xA1B2C3D4,2,4,0,0,0x00040000,1 };
...
static void DrawPacketStatistics(const HookPacket& hp, const std::vector<SegmentInfo>& segs) {
    if (ImGui::CollapsingHeader("Packet Statistics", ImGuiTreeNodeFlags_DefaultOpen)) {
        if (ImGui::BeginTable("pkt_stats", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
            ...
            size_t ipcCount = 0;
            size_t compressedSize = hp.len > 0x28 ? (hp.len - 0x28) : 0;
            ...
            if (P.isCompressed && totalPayloadSize > 0) {
                ...
                std::ostringstream os; os << std::fixed << std::setprecision(1) << (ratio * 100.0f) << "%";
            ...
            if (!opcodeCounts.empty()) {
                std::vector<std::pair<uint16_t, int>> sorted(opcodeCounts.begin(), opcodeCounts.end());
                std::sort(sorted.begin(), sorted.end(), [](auto& a, auto& b) { return a.second > b.second; });
                std::ostringstream opcodes;
                int shown = 0;
                for (const auto& [op, count] : sorted) {
                    if (shown++ >= 5) { opcodes << ", ..."; break; }
...
static void DrawPacketListAndDetails(const std::vector<HookPacket>& display) {
    ...
    if (selectedFiltered >= 0 && selectedFiltered < (int)filtered.size()) {
        ...
        if (ImGui::Button("Export JSON")) {
            ...
            ImGuiFD::OpenDialog("Export JSON", ImGuiFDMode_SaveFile, "exports", "{JSON Files:*.json}, {*.*}");
        }
        ImGui::SameLine();
        if (ImGui::Button("Export PCAP")) {
            s_pendingPcap = hp;
            s_openPcapDialog = true;
            ImGuiFD::OpenDialog("Export PCAP", ImGuiFDMode_SaveFile, "exports", "{PCAP Files:*.pcap}, {*.*}");
        }
        ImGui::Separator();
    }
    else {
