			...
			scanFuture = std::async(std::launch::async, [this]() {
				...
				try {
					...
					auto stringHits = m_functionScanner->ScanMemoryForFunctionStrings({
						"Actor","UI","Addon",
						"Agent","Network","Packet","Ability","Render","Socket"
						});

					// Collect function addresses near string hits
					std::vector<uintptr_t> stringAddrs;
					stringAddrs.reserve(stringHits.size());
					for (const auto& h : stringHits) {
						if (h.nearbyFunctionAddress) stringAddrs.push_back(h.nearbyFunctionAddress);
					}
					...
					all.reserve(results.size() + stringAddrs.size());
					for (auto a : results) all.insert(a);
					for (auto a : stringAddrs) all.insert(a);
