...
namespace {
	...
	static void CollectAsciiStrings(uint8_t* rdata, size_t rdataSize, size_t minLen, size_t limit,
		...
		out.reserve(std::min<size_t>(limit, 20000));
		std::string cur;
	...
	// Budgeted, cancellable mapping of strings -> functions via RIP-relative refs.
	static XrefResult BuildStringXrefsFallback(FunctionCallMonitor* self,
		HMODULE mod,
		int minLen,
		size_t maxStringsPerFn,
		std::atomic<bool>& cancel,
		std::chrono::milliseconds budget = std::chrono::milliseconds(120000)) {
		XrefResult res{};
		if (!mod) { return res; }
		...
		// 1) Collect strings (bounded). Tune the limits for speed vs. coverage.
		const size_t asciiLimit = 25000;   // stop after N ascii strings
		const size_t utf16Limit = 8000;    // stop after N utf16 strings
		std::vector<std::pair<uintptr_t, std::string>> asciiStrings;
		std::vector<std::pair<uintptr_t, std::string>> utf16Strings;

		CollectAsciiStrings(rdata, rdataSize, static_cast<size_t>(std::max(3, minLen)), asciiLimit, asciiStrings);
		CollectUtf16Strings(rdata, rdataSize, static_cast<size_t>(std::max(3, minLen)), utf16Limit, utf16Strings);
		res.asciiCount = asciiStrings.size();
		res.utf16Count = utf16Strings.size();

		// 2) Map strings to functions via RIP-relative references.
		std::unordered_map<uintptr_t, std::vector<std::string>> func2strs;
		func2strs.reserve(4096);

		auto mapOneBatch = [&](const std::vector<std::pair<uintptr_t, std::string>>& list) {
			...
			for (const auto& kv : list) {
				...
				// Find RIP-relative references into .text
				auto refs = SapphireHook::PatternScanner::FindRipReferencesTo(mod, saddr);
				if (refs.empty()) continue;

				// Cap refs per string to keep it fast
				size_t refCap = 4;
				size_t taken = 0;
				for (auto ref : refs) {
					if (taken >= refCap) break;
					uintptr_t fn = self->FindFunctionStart(ref);
					if (!fn) continue;

					auto& vec = func2strs[fn];
					// prevent duplicates
		...
		mapOneBatch(asciiStrings);
		if (!cancel.load(std::memory_order_relaxed) &&
			std::chrono::steady_clock::now() - start < budget) {
			mapOneBatch(utf16Strings);
		}

		// 3) Build rows, cap strings per function.
		res.rows.reserve(func2strs.size());
		for (auto& [fn, strs] : func2strs) {
			XrefRow r;
			r.addr = fn;
			r.name = self->ResolveFunctionName(fn);
			if (r.name.empty()) {
				char buf[24]; std::snprintf(buf, sizeof(buf), "0x%016llX", static_cast<unsigned long long>(fn));
				r.name = buf;
			}
			if (strs.size() > maxStringsPerFn) {
				strs.resize(maxStringsPerFn);
			}
			r.strings = std::move(strs);
			res.rows.emplace_back(std::move(r));
		}

		// Sort by number of strings desc for nicer display
...
void FunctionCallMonitor::RenderWindow()
{
	...
	if (ImGui::Begin(GetDisplayName(), &m_windowOpen))
	{
		if (ImGui::BeginTabBar("MainTabs"))
		{
			...
			if (ImGui::BeginTabItem("Strings/XRefs"))
			{
				RenderStringXrefTab();
				ImGui::EndTabItem();
			}
...
void FunctionCallMonitor::RenderStringXrefTab()
{
	ImGui::TextUnformatted("Scan .rdata for ASCII/UTF-16 strings and map references to functions (.text).");
	ImGui::Separator();

	ImGui::SetNextItemWidth(120);
	ImGui::InputInt("Min string length", &m_strXref.minLen);
	if (m_strXref.minLen < 3) m_strXref.minLen = 3;

	ImGui::SameLine();
	ImGui::SetNextItemWidth(140);
	int maxPer = static_cast<int>(m_strXref.maxStringsPerFn);
	if (ImGui::InputInt("Top strings per function", &maxPer)) {
		if (maxPer < 1) maxPer = 1;
		if (maxPer > 10) maxPer = 10;
		m_strXref.maxStringsPerFn = static_cast<size_t>(maxPer);
	}

	if (!m_strXref.running) {
		if (ImGui::Button("Scan strings and build XREFs")) {
			m_strXref.running = true;
			m_strXref.cancel.store(false, std::memory_order_relaxed);
			m_strXref.status.clear();
			m_strXref.rows.clear();
			m_strXref.totalAscii = m_strXref.totalUtf16 = 0;
			m_strXref.started = std::chrono::steady_clock::now();

			m_strXref.task = std::async(std::launch::async, [this]() {
				try {
					HMODULE selfMod = GetModuleHandleW(nullptr);
					auto res = BuildStringXrefsFallback(this,
						selfMod,
						m_strXref.minLen,
						m_strXref.maxStringsPerFn,
						m_strXref.cancel,
						std::chrono::milliseconds(120000)); // 120s budget
					m_strXref.totalAscii = res.asciiCount;
					m_strXref.totalUtf16 = res.utf16Count;
					m_strXref.rows.clear();
					m_strXref.rows.reserve(res.rows.size());
					for (auto& rr : res.rows) {
						StringXrefState::Row row;
						row.addr = rr.addr;
						row.name = std::move(rr.name);
						row.strings = std::move(rr.strings);
						m_strXref.rows.emplace_back(std::move(row));
					}
					m_strXref.status = "Done";
				}
				catch (const std::exception& e) {
					m_strXref.status = std::string("Scan failed: ") + e.what();
				}
				catch (...) {
					m_strXref.status = "Scan failed (unknown error)";
				}
				m_strXref.running = false;
				});
		}
	}
	else {
		// Cancel and timeout controls
		if (ImGui::Button("Cancel")) {
			m_strXref.cancel.store(true, std::memory_order_relaxed);
		}
		ImGui::SameLine();
		const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
			std::chrono::steady_clock::now() - m_strXref.started).count();
		ImGui::TextDisabled("Scanning... %llds", static_cast<long long>(elapsed));
		if (elapsed > 120) {
			ImGui::TextColored(ImVec4(1, 0.7f, 0, 1), "Timeout budget exceeded. Results will be partial.");
		}
	}

	if (!m_strXref.status.empty()) {
		ImGui::Text("Status: %s | ASCII: %zu, UTF16: %zu",
			m_strXref.status.c_str(), m_strXref.totalAscii, m_strXref.totalUtf16);
	}

	if (!m_strXref.rows.empty()) {
		ImGui::Separator();
		if (ImGui::BeginTable("str_xref_tbl", 3, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_SizingStretchProp)) {
			ImGui::TableSetupColumn("Address", ImGuiTableColumnFlags_WidthFixed, 180.0f);
			ImGui::TableSetupColumn("Function");
			ImGui::TableSetupColumn("Top strings");
			ImGui::TableHeadersRow();

			for (const auto& r : m_strXref.rows) {
				ImGui::TableNextRow();
		...
		if (ImGui::Button("Import labels (XRefStr:...) to Function DB")) {
			size_t added = 0;
			for (const auto& r : m_strXref.rows) {
				if (r.strings.empty()) continue;
				std::string existing = m_functionDB.GetFunctionName(r.addr);
				if (!existing.empty()) continue;
				std::string label = "XRefStr:" + r.strings.front();
				m_functionDB.AddFunction(r.addr, label, "String XREF", "Strings");
				++added;
			}
			LogInfo("String XREF import: added " + std::to_string(added) + " labels to Function DB");
		}
	}
}
	...
	const std::vector<std::string>* tags)
{
	...
	if (startHeur != address) {
		oss << "Heuristic Start: " << Hex64(startHeur)
			<< " (offset -" << std::dec << (address - startHeur) << " bytes)\n";
	}
	else {
