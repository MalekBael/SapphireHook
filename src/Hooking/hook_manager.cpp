...
static bool ApplyRetainerClassJobCachePatch() {
  if (IsEnvEnabled("SAPPHIRE_DISABLE_RETAINER_CLASSJOB_PATCH")) {
    LogWarning("Retainer classJob cache patch disabled via environment");
    return false;
  }
  ...
  if (!GetMainModuleInfo(moduleBase, moduleSize)) {
    LogError("Retainer classJob cache patch: failed to get module info");
    return false;
  }

  // Sequence in sub_1404DAFE0 (3.35):
  // movzx eax, byte [rdi + 0x29]
  // mov byte [rcx + rsi + 0x2126], al
  const char *pattern = "0F B6 47 29 88 84 31 26 21 00 00 8B 47 2C";
  const uintptr_t match = patternscan(moduleBase, moduleSize, pattern);
  if (!match) {
    LogError("Retainer classJob cache patch: pattern not found");
    return false;
  }

  const uintptr_t movByteAddr = match + 4; // start of 88 84 31 26 21 00 00
  if (!IsValidMemoryAddress(movByteAddr, 7)) {
    LogError("Retainer classJob cache patch: target not readable");
    return false;
  }

  const uint8_t expected[7] = {0x88, 0x84, 0x31, 0x26, 0x21, 0x00, 0x00};
  const auto *current = reinterpret_cast<const uint8_t *>(movByteAddr);
  if (!std::equal(std::begin(expected), std::end(expected), current)) {
    std::ostringstream oss;
    oss << "Retainer classJob cache patch: bytes mismatch at 0x" << std::hex
        << movByteAddr;
    LogError(oss.str());
    return false;
  }

  SafeMemoryRegion region(movByteAddr, sizeof(expected));
  if (!region.IsValid()) {
    LogError("Retainer classJob cache patch: SafeMemoryRegion invalid");
    return false;
  }

  auto span = region.GetWritableSpan();
  if (span.size() < sizeof(expected)) {
    LogError("Retainer classJob cache patch: writable span too small");
    return false;
  }

  span[3] = 0x27; // change displacement from 0x2126 to 0x2127
  LogInfo("Retainer classJob cache patch applied (0x2126 -> 0x2127)");
  return true;
}
  ...
  try {
    ...
    for (int j = 0; j < 50; ++j) {
      if (code[j] == 0x0F && code[j + 1] == 0xB7) {
        looksLikeIPC = true;
        LogInfo("Found movzx at +" + std::to_string(j));
        break;
      }
      if (code[j] == 0x66 && (code[j + 1] == 0x81 || code[j + 1] == 0x83)) {
        looksLikeIPC = true;
        LogInfo("Found 16-bit cmp at +" + std::to_string(j));
        break;
      }
      if (code[j] == 0x48 && code[j + 1] == 0x8D && code[j + 2] == 0x15) {
        looksLikeIPC = true;
        LogInfo("Found LEA at +" + std::to_string(j));
        break;
      }
    }

    if (looksLikeIPC)
      LogInfo("IPC handler validation passed");
    else
      LogWarning("IPC handler validation failed");
    return looksLikeIPC;
  } catch (...) {
...
bool FindIPCByOpcodeReferences(uintptr_t moduleBase, size_t moduleSize) {
  try {
    ...
    for (uint16_t opcode : knownOpcodes) {
      ...
      {
        std::ostringstream oss;
        oss << "Searching opcode 0x" << std::hex << opcode;
        LogInfo(oss.str());
      }
      for (uintptr_t addr = moduleBase; addr < moduleBase + moduleSize - 4;
           addr += 4) {
        ...
        for (uintptr_t scan = backStart; scan < addr; ++scan) {
          ...
          if ((c[0] == 0x48 && c[1] == 0x89) ||
            ...
            ipcHandlerAddr = scan;
            std::ostringstream oss;
            oss << "Found potential IPC handler @0x" << std::hex << scan;
            LogInfo(oss.str());
            return true;
          }
        }
    ...
    LogError(std::string("Exception in FindIPCByOpcodeReferences: ") +
             e.what());
    return false;
  }
}
...
void HookStatistics::Update(const std::vector<HookInfo> &hooks) {
  ...
  for (auto &h : hooks) {
    if (h.is_enabled)
      ...
      ++disabledHooks;
    if (!h.validation_error.empty())
      ++failedHooks;
    hooksByModule[h.module_name]++;
    hooksByAssembly[h.assembly_name]++;
  }
...
std::string HookStatistics::ToDebugString() const {
  ...
  oss << "Hook Statistics:\n"
      ...
      << "  Failed: " << failedHooks << "\n"
      << "  By Module:\n";
  for (auto &m : hooksByModule)
    oss << "    " << m.first << ": " << m.second << "\n";
  oss << "  By Assembly:\n";
  for (auto &a : hooksByAssembly)
...
bool ValidateHookTarget(uintptr_t address, const std::string &name) {
  if (!HookManager::ValidateHookAddress(address)) {
    std::ostringstream oss;
    oss << "Hook target invalid: " << name << " 0x" << std::hex << address;
    LogError(oss.str());
    return false;
  }
  if (HookManager::IsAddressHooked(address)) {
    std::ostringstream oss;
    oss << "Already hooked: " << name << " 0x" << std::hex << address;
    LogWarning(oss.str());
    return false;
  }
...
bool InstallIPCHookSafe(uintptr_t address, const std::string &name) {
  {
    std::ostringstream oss;
    oss << "Starting safe hook installation for " << name << " at 0x"
        << std::hex << address;
    LogInfo(oss.str());
  }
  ...
  }
  {
    ...
    oss << "Memory info " << name << ": State=" << std::dec << mbi.State
        << " Protect=0x" << std::hex << mbi.Protect << " Type=" << std::dec
        << mbi.Type;
    LogInfo(oss.str());
  }
  ...
  try {
    MH_STATUS createRes =
        MH_CreateHook(reinterpret_cast<void *>(address), &HookedHandleIPC,
                      reinterpret_cast<void **>(&originalHandleIPC));
    if (createRes != MH_OK) {
      LogError("MH_CreateHook failed for " + name +
               " err=" + std::to_string(createRes));
      return false;
    }
    ...
    if (enableRes != MH_OK) {
      LogError("MH_EnableHook failed for " + name +
               " err=" + std::to_string(enableRes));
    ...
    {
      std::ostringstream oss;
      oss << "🎉 Installed hook " << name << " at 0x" << std::hex << address;
      LogInfo(oss.str());
    }
    return true;
  } catch (const std::exception &e) {
...
extern "C" __declspec(noinline) void __fastcall CallOriginalIPC_NoExcept(
    void *thisPtr, uint32_t actorId, void *packetData) {
  __try {
    originalHandleIPC(thisPtr, actorId, packetData);
  } __except (EXCEPTION_EXECUTE_HANDLER) {
  }
}

extern "C" __declspec(noinline) bool __fastcall ProbeDispatcherOpcode_NoExcept(
    void *rcx, uint8_t *outOpcode) {
  __try {
...
ReadPacketOpcode_NoExcept(void *packetData, uint16_t *outOpcode) {
  __try {
    *outOpcode = *reinterpret_cast<uint16_t *>(
...
void __fastcall HookedHandleIPC(void *thisPtr, uint32_t actorId,
                                void *packetData) {
  s_cachedIpcThisPtr.store(thisPtr, std::memory_order_relaxed);

  uint16_t opcode = 0;
  (void)ReadPacketOpcode_NoExcept(packetData, &opcode);
  ...
  g_totalExecMicros.fetch_add(static_cast<uint64_t>(dur.count()),
                              std::memory_order_relaxed);

  static const bool verboseLog = IsEnvEnabled("SAPPHIRE_IPC_LOG");
  static const bool warnSlow = IsEnvEnabled("SAPPHIRE_IPC_WARN_SLOW");
  static const bool warnSecurity = IsEnvEnabled("SAPPHIRE_IPC_WARN_SECURITY");

  if (verboseLog || warnSlow || warnSecurity) {
    const char *opcodeName = GetOpcodeName(opcode);

    if (verboseLog) {
      void *retAddr = _ReturnAddress();
      std::ostringstream ctx;
      ctx << "IPC[" << opcodeName << "](0x" << std::hex << opcode
          << ") actor=0x" << actorId << " from 0x"
          << reinterpret_cast<uintptr_t>(retAddr);
      LogInfo(ctx.str());
    }

    if (warnSecurity &&
        (opcode == 0x00DE || opcode == 0x026D || opcode == 0x010F)) {
      LogWarning(std::string("SECURITY: Kick/Ban opcode detected: ") +
                 opcodeName);
    }

    if (warnSlow && dur.count() > 1000) {
      LogWarning(std::string("Slow IPC operation: ") + opcodeName + " took " +
                 std::to_string(dur.count()) + "µs");
    }
  }
}
...
char __fastcall HookedDispatcher(void *rcx) {
  ...
  if (ok) {
    try {
      ...
      log << "[Dispatcher] Opcode: 0x" << std::hex << std::setw(2)
          << std::setfill('0') << static_cast<int>(opcode) << "\n";
    } catch (...) {
    }
    if (opcode == 0xDE)
      LogWarning("PcPartyKick (0xDE) triggered");
  } else {
...
bool FindAndHookIPC() {
  if (IsEnvEnabled("SAPPHIRE_ONLY_WSASEND") || IsEnvEnabled("SAPPHIRE_SAFE") ||
  ...
  // sub_140DD9430: void __fastcall HandleIPC(__int64 a1, uint a2, __int64
  // packetData) Opcode read: movzx edx, word [r8 + 2] ->
  // switch(*(_WORD*)(a3+2))
  const char *patterns[] = {
  ...
  for (int i = 0; i < patternCount; ++i) {
    ...
    {
      std::ostringstream oss;
      oss << "Found potential IPC handler via pattern " << (i + 1) << " @0x"
          << std::hex << addr;
      LogInfo(oss.str());
    }
...
bool FindAndHookDispatcher() {
  ...
  const char *dispatcherPatterns[] = {
      "48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 54 41 55 41 56 41 57 48 "
      "83 EC ? 45 33 FF",
  ...
  const int count = static_cast<int>(sizeof(dispatcherPatterns) /
                                     sizeof(dispatcherPatterns[0]));
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (!HookManager::ValidateHookAddress(addr)) {
      ...
      oss2 << "Rejected dispatcher addr 0x" << std::hex << addr;
      LogWarning(oss2.str());
      continue;
    }
    dispatcherAddr = addr;
...
void HookManager::Initialize() {
  ...
  try {
    ...
    if (!ApplyRetainerClassJobCachePatch())
      LogWarning("Retainer classJob cache patch not applied");
...
bool HookManager::ValidateHookAddress(uintptr_t address) {
  if (!IsCanonicalUserVA(address)) {
    std::ostringstream oss;
    oss << "[HookManager] Reject non-canonical 0x" << std::hex << address;
    LogError(oss.str());
    return false;
  }
  uintptr_t base{};
...
bool HookManager::LoadHookCache() {
  ...
  try {
    ...
    std::ostringstream oss;
    oss << "Loaded " << loaded << " cached hook addresses";
    LogInfo(oss.str());
    return loaded > 0;
  } catch (const std::exception &e) {
...
uintptr_t HookManager::AddressToRVA(uintptr_t address) {
  ...
  if (mainModule) {
    MODULEINFO mi{};
    if (GetModuleInformation(GetCurrentProcess(), mainModule, &mi,
                             sizeof(mi))) {
...
const char *GetOpcodeName(uint16_t opcode) {
  // IMPORTANT:
  // Many opcodes overlap numerically between client->server and server->client.
  // For example 0x01B0 is "ItemSize" (server) but "HousingExteriorChange"
  // (client). This IPC hook is invoked from the game's IPC handler, so we
  // default to server-side zone names, but if both are known we disambiguate to
  // avoid misleading logs.
  const char *serverZone =
      ::LookupOpcodeName(opcode, false, Net::ConnectionType::Zone);
  const char *clientZone =
      ::LookupOpcodeName(opcode, true, Net::ConnectionType::Zone);

  const bool serverKnown = (serverZone && serverZone[0] != '?');
  const bool clientKnown = (clientZone && clientZone[0] != '?');
  ...
  if (serverKnown && clientKnown) {
    if (std::strcmp(serverZone, clientZone) == 0)
      return serverZone;

    thread_local std::string combined;
    combined.clear();
    combined.reserve(64);
    combined += "Server:";
    combined += serverZone;
  ...
  // Fall back: try chat tables too, then unknown.
  if (const char *serverChat =
          ::LookupOpcodeName(opcode, false, Net::ConnectionType::Chat);
      serverChat && serverChat[0] != '?')
    return serverChat;
  if (const char *clientChat =
          ::LookupOpcodeName(opcode, true, Net::ConnectionType::Chat);
      clientChat && clientChat[0] != '?')
    return clientChat;
...
extern "C" __declspec(noinline) bool InjectServerPacket_SEH(HandleIPC_t handler,
                                                            void *thisPtr,
                                                            uint32_t actorId,
                                                            void *packetData) {
  __try {
...
bool InjectServerPacket(uint16_t opcode, const void *payload,
                        size_t payloadSize) {
  // Get cached thisPtr from previous IPC calls
  void *cachedThis = s_cachedIpcThisPtr.load(std::memory_order_relaxed);
  if (!cachedThis) {
    LogError("[InjectServerPacket] No cached IPC thisPtr - wait for game to "
  ...
  *reinterpret_cast<uint16_t *>(&buffer[2]) = opcode;
  ...
  bool result =
      InjectServerPacket_SEH(originalHandleIPC, cachedThis, 0, buffer.data());
...
void InitHooks() { SapphireHook::HookManager::Initialize(); }